...
2. **GAS pipes: DONE (2026-06-05, in-game verified).** `CC_GasPipe` + hydrogen/CO₂ test rig, pure asset clone of the fluid pattern (`docs/plans/2026-06-05-gas-channel-assets-design.md`); final gas art replaces the placeholder `GasTube_*`/`gastube_*` files in place. **ITEM pipes (§13.4): IMPLEMENTED + in-game verified 2026-06-07.** Discrete traveling stacks against vanilla containers (Mekanism rules: PULL extracts, nearest-first routing, re-route→return→pop-out ladder, no extraction without a destination), metadata round-tripped through delivery, filter stub at extraction/destination/junctions for the future intersection filters, `CC_ItemPipe` (single texture, no on-state), chest arms via programmatic visual masks (effective > physical adds arms; the swap SETS rotation so arms point AT the endpoint), wrench cycles chest-pointing faces, item-network panel rows, in-transit wipe survival. v1 endpoints = vanilla containers; machine ITEM ports join at the machines milestone. `docs/plans/2026-06-06-item-channel-design.md`. **NEUTRON pipes (§13.5):** own later plan.

**Blocks can be placed on pipes (2026-06-07 fix):** chests (and any support-requiring block) could not be placed on/against a pipe. Root cause: the engine only auto-grants `ALL_SUPPORTING_FACES` to `DrawType` `Cube`/`CubeWithModel`/`GizmoCube` + `Solid` blocks (`BlockType.processConfig`); our pipes are `DrawType: Model`, so they offered ZERO supporting faces, and a chest's `Support.Down = Full` requirement failed against them. Vanilla chests are `Model` too and compensate with an explicit `Supporting` map: the four pipe BlockTypes now declare `Supporting: Full` on all six cube faces (they collide as a full cube, so they support neighbours on any face like a solid block). Regression-guarded by `PipeSupportingFacesTest`. In-game verification pending.

**Bare pipe ends are wrench-configurable (2026-06-07):** the full `NORMAL→PUSH→PULL→NONE` ring is now offered on any pipe face that does NOT touch another pipe (air, a solid block, or an endpoint); only pipe↔pipe faces keep the 2-state `NORMAL↔NONE` ring. A player can pre-configure a bare end to PUSH/PULL and it persists in `PipeNode`'s codec, activating the arm + tip automatically (via the existing `PlaceBlockEvent` invalidation) when a chest/machine is later placed there. Before then the panel "Faces:" row is the feedback (a disconnected end has no arm for a tip). Pure rule lives in `WrenchCycles.targetForNeighbour`; the 2-PUSH/PULL-face budget still applies. `docs/plans/2026-06-05-pipe-flow-states-design.md`.

**Push/pull tip rendering + connection rules (§13.9): IMPLEMENTED 2026-06-07, ITEM family shipped.** Per-pipe wrench budget of 2 PUSH/PULL faces (the connection cap was removed: superseded). Generated composite models put a push/pull tip on any arm of any shape (`scripts/gen_pipe_tips.py`: ~604 states/family, derived from the authored tip geometry, no hand-authoring). `CC_ItemPipe` ships the full tip set; **Power/Fluid/Gas families + a rotated-collar UV polish are DEFERRED** as one "finish tip rollout" task (the collar needs an authored EAST orientation as ground truth: see `docs/plans/2026-06-07-tip-rendering-design.md` "DEFERRED"). The overlay-entity approach to per-face indicators was spiked and found DEAD in 0.5.3 (the client raycasts any networked model entity: no click-through).
...
- **Visuals are programmatic.** The engine's pattern system **cannot see flow state**, so any pipe with a suppressed, added, or tipped arm gets **programmatic shape states**; all-`NORMAL`, unmismatched pipes keep riding the pattern system for free. A suppressed arm (`NONE`/mismatch) renders the **lesser topology** shape (tee → straight, straight → end: all 26 shapes already exist); a container arm the engine refuses to weld is **added** (effective > physical). The programmatic swap **sets rotation** (entity-preserving `setBlock`, settings `198`) so added/tipped arms point at their endpoint, and a position-tracking set actively **resets** an overridden pipe to plain when it returns to undisturbed (the engine never re-resolves on a flow/container change). `PUSH`/`PULL` endpoint arms render **generated composite tip models** (`scripts/gen_pipe_tips.py`, ~604+ states/family) on ANY shape, bounded by the 2-PUSH/PULL-faces-per-pipe wrench budget: this **supersedes** the earlier end-stub-only indicators (which only rendered on lone single-arm ends). ITEM ships the full tip set; other families + a rotated-collar UV polish are deferred (§0 status; `docs/plans/2026-06-07-tip-rendering-design.md`). The overlay-entity alternative for tips was spiked and found DEAD in 0.5.3 (clients raycast any networked model entity, blocking click-through).
