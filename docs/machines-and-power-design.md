...
2. **GAS pipes: DONE (2026-06-05, in-game verified).** `CC_GasPipe` + hydrogen/CO₂ test rig, pure asset clone of the fluid pattern (`docs/plans/2026-06-05-gas-channel-assets-design.md`); final gas art replaces the placeholder `GasTube_*`/`gastube_*` files in place. **ITEM pipes (§13.4): IMPLEMENTED 2026-06-06 (in-game verification pending, combined with flow-states).** Discrete traveling stacks against vanilla containers (Mekanism rules: PULL extracts, nearest-first routing, re-route→return→pop-out ladder, no extraction without a destination), filter stub at extraction/destination/junctions for the future intersection filters, `CC_ItemPipe` (32 states, single texture, no on-state), chest arms via programmatic visual masks (effective > physical adds arms), wrench cycles chest-pointing faces, item-network panel rows, in-transit wipe survival. v1 endpoints = vanilla containers; machine ITEM ports join at the machines milestone. `docs/plans/2026-06-06-item-channel-design.md`. **NEUTRON pipes (§13.5):** own later plan.
