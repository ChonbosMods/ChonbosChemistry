...
- D48. Inline `CC_*Filter` blocks (one per matter channel: ITEM/FLUID/GAS) splice into a pipe run and pass a resource only if its substance matches a configurable whitelist/blacklist: ITEM gates traveling stacks (sorter-lite, no active branch-sorting in v1), FLUID/GAS act as a conditional network boundary moving only matching substance across. (§6.3)
...
- D49. Solids get a bulk container (`CC_SolidBin`): one solid substance collapsed to a count, locked-until-empty, three capacity tiers, carries contents: the D30 tank rules for solids; fed/drained by ITEM pipe or carry, accepts only fungible `Chem_Solid_*` substance items. (§7.5)
...
- **Substance filter (`CC_*Filter`, D48):** an inline filter *block* that splices into a pipe run on the three matter channels (ITEM/FLUID/GAS; POWER/NEUTRON are fungible currencies with no substance identity, so there is nothing to filter). It connects via the channel family's FaceTags exactly like a pipe, so there is one variant per matter channel (`CC_ItemFilter`/`CC_FluidFilter`/`CC_GasFilter`). It holds a configurable substance whitelist/blacklist (substance ids and/or tags) and passes a resource across only if it matches.
  - **ITEM:** gates `TravelingStack`s: a stack whose substance is rejected is not routed across that node (a lightweight Mekanism logistical-sorter; routing stays nearest-first, the filter only admits/denies). It does not actively sort onto branches in v1 (that is the deferred splitter, §11).
  - **FLUID/GAS:** because these are fungible, type-locked, single-substance shared buffers (D15/D18), the filter is a **conditional network boundary**: it splits the run into two cached sub-networks and moves only matching substance across (the same simulate->commit transfer a machine port uses), so each side keeps its own single-substance buffer. With type-lock this is the tool that pre-commits a branch to an intended substance (a substance-selective valve).
  - Configured by the wrench or a small block GUI (same clicked-face / `UseLatestTarget` plumbing as the machine wrench). [DECIDE] whitelist-vs-blacklist default and the config-UI surface (§11).
...
- **The only new code:** `BlockModule.getBlockEntity` returns null on a filler, so our **network endpoint collection and the wrench apply the filler->anchor one-liner** before reading the anchor's component. FaceTags are uniform across a machine's cells (they cannot differ per footprint cell), so they only mean "a pipe of this channel may stub here": the real channel/direction filtering lives in our `PortConfig`, keyed by **(footprint-cell-offset, face)**, resolved on the anchor. The wrench reads the raw clicked cell + face from the `InteractionContext` to target the right port even though `interactWithBlock` receives the anchor.
  - **IMPLEMENTED 2026-06-20 (CC_Smelter, confirmed in-game):** `Port` carries a model-space cell offset; `PortConfig.forCell` selects one cell's ports; `WorldMachineLookup.at` resolves filler→anchor (`getFiller`+`FillerBlockUtil.unpack`) and returns per-cell `ports()` (transport + effective visual mask) plus anchor-level `advertisesChannel()` (physical mask = engine-weld mirror); `PortProjection.forWorldCell` rotates model→world via engine `Rotation.rotateY`. Inherited-tag arms on portless cells/faces (`effective < physical`) are dropped by the existing suppressed-arm override. See [plans/2026-06-20-cc-smelter-footprint-facetag-research.md](plans/2026-06-20-cc-smelter-footprint-facetag-research.md) §RESOLVED and [[filler-cells-inherit-anchor-facetags]].
...
- **Solid storage (`CC_SolidBin`, D49):** solids get the bulk store the tanks give fluids/gases. A solid container holds a quantity of ONE solid substance collapsed to a count (not discrete stacks): substance-locked until empty, three capacity-only tiers, not stackable, relocates with contents: the D30 tank rules applied to solids. Fed/drained only by an ITEM pipe network or by carrying the whole block; it accepts only fungible substance items (the metadata-free `Chem_Solid_*` jars, §3.4), which is exactly what lets it collapse them to a count and distinguishes it from a vanilla chest (discrete, identity-bearing stacks). The mental model extends: tanks for fluids/gases, bins for solids, in sizes.
...
13. **Substance filter UX** (§6.3, D48): whitelist-vs-blacklist default, the config-UI surface (wrench cycle vs a small GUI), and the deferred active per-substance *splitter* (filter admits/denies in v1; sorting a mixed ITEM stream onto branches by substance is a later device).
