...
**VERDICT (revised 2026-06-07 after the in-game probe): DEAD in Server 0.5.3 for click-through overlays.** The decompile half held up (rendering, scale, non-persistence all confirmed in-game via `/cc-tipspike`), but the kill criterion failed: the probe blocked attack/Interact even when spawned with NO `BoundingBox`, NO `HitboxCollision`, and NO `Interactable`. The CLIENT raycasts the model geometry of any networked entity when resolving the crosshair target; the server-side opt-in components only decide what happens afterwards, so the input is swallowed (server finds nothing interactable) instead of falling through to the block behind. `PropUpdate` is a zero-field marker packet and `ModelUpdate` carries only scale + model: there is NO wire-protocol flag to exempt an entity from client targeting. Revisit when client modding APIs or a protocol targetability flag appear. The recipe below remains valid for overlay uses where click-through does not matter (holograms, machine status props).

*Original decompile verdict (superseded): VIABLE-WITH-CAVEATS.* A static prop entity renders an arbitrary blockymodel at sub-block coords with full rotation + scale, and can be made non-persistent with one built-in marker. The model must be a **registered `ModelAsset`**, not a raw path. Best precedent: the engine's deployable entity.
