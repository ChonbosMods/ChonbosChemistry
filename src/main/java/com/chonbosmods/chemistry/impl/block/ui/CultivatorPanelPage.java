...
/**
 * READ-ONLY panel for the CC Cultivator ({@code Pages/CC_CultivatorPanel.ui}). It is the sibling of
 * {@link BenchMachinePanelPage} (Smelter/Reclaimer) but reads a {@link CultivatorState} rather than a
 * {@code MachineBlockState}: the Cultivator is an AUTONOMOUS crafter, so it owns its input/output containers,
 ...
 *
 * <p>The left "input" grid is NOT a push-fed input buffer: the Cultivator is a demand-driven puller, so its
 ...
 * {@link CultivatorState#setEnabled(boolean)}) and an Eject button draining the Cultivator's own input + output
 ...
 * {@link CultivatorTickSystem#craftDurationFor} (the same per-recipe {@code getTimeSeconds}, or
 * {@code CULTIVATOR_DEFAULT_DURATION} fallback, the tick times the craft by). So the bar fills exactly over each
 ...
 * on the START tick the ingredients are pulled but progress is still 0, yet the Cultivator IS crafting, so
 ...
 */
public final class CultivatorPanelPage extends InteractiveCustomUIPage<CultivatorPanelPage.PageData>
        implements PanelRefreshService.LivePanel {

    @Nonnull
    private final Ref<ChunkStore> blockRef;
    /** Cultivator display name shown in the title bar. */
    @Nonnull
    ...
    public void build(
        ...
        commandBuilder.append("Pages/CC_CultivatorPanel.ui");
    ...
    public boolean refresh() {
        ...
        if (this.cultivator() == null) {
            this.close(); // cultivator broken mid-view: auto-close
            return false;
        }
    ...
    private void applyAction(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull String action) {
        CultivatorState cultivator = this.cultivator();
        if (cultivator == null) {
            return;
        }
        if (Action.TOGGLE.equals(action)) {
            cultivator.setEnabled(!cultivator.isEnabled());
            this.markNeedsSaving();
        } else if (Action.EJECT.equals(action)) {
            this.eject(ref, store, cultivator);
            this.markNeedsSaving();
        } else {
            return;
        }
    ...
    /** Resolve the Cultivator's {@link CultivatorState}, or null if the block is gone / not a Cultivator. */
    private CultivatorState cultivator() {
        if (!this.blockRef.isValid()) {
            return null;
        }
        return this.blockRef.getStore()
            .getComponent(this.blockRef, ChonbosChemistry.getInstance().cultivatorComponentType());
    }

    /** Populate every panel selector from the current Cultivator state. Null-safe throughout. */
    private void applyState(@Nonnull UICommandBuilder cmd) {
        CultivatorState cultivator = this.cultivator();
        if (cultivator == null) {
            return;
        }
        boolean enabled = cultivator.isEnabled();
        ...
        float progressSeconds = cultivator.progress();
        boolean active = cultivator.currentRecipeId() != null;
        float duration = CultivatorTickSystem.craftDurationFor(cultivator.currentRecipeId());
     ...
     * the held ingredients list so an idle Cultivator renders an empty list and a crafting Cultivator renders just the
    ...
    /** Drain the Cultivator's own input + output containers into the player's inventory; drop overflow at feet. */
    private void eject(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull CultivatorState cultivator) {
        ...
        List<ContainerView> sources = List.of(
            new ItemContainerView(cultivator.held()),
            new ItemContainerView(cultivator.output()));
        ...
        // keeps both the ingredients and the finished item (item duplication). Resetting here drops the Cultivator
        ...
        cultivator.setCurrentRecipeId(null);
        cultivator.setProgress(0f);
        ...
        ItemStack card = cultivator.card();
        if (card != null && !ItemStack.isEmpty(card)) {
            ...
            cultivator.setCard(null);
            cultivator.clearScriptProgress();
        }
        ...
        if (transform == null || transform.getPosition() == null) {
            return; // can't place a drop without a position; the items stay in the Cultivator
        }
    ...
    /** Mark the Cultivator's chunk needing-save after a toggle/eject (WrenchInteraction pattern). */
