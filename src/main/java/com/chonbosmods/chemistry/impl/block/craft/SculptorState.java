...
/**
 * The persistent ECS state the Sculptor block carries on the {@link ChunkStore}. Unlike the processing
 * machines (Smelter/Reclaimer) whose {@code MachineBlockState} HOLDS a vanilla
 * {@code ProcessingBenchBlock} to borrow its input/output containers, the Sculptor has no vanilla bench
 * block to delegate to: it is an autonomous crafter and therefore owns its own containers and craft
 * state here. (A direct mirror of {@code AlembicState}; the Sculptor differs from the Alembic only in its
 * footprint/ports + its recipe pool + its recipe-script GATE, all of which live in {@code SculptorTickSystem}.)
 *
 * <h2>Fields</h2>
 * <ul>
 *   <li>{@code energy}: the power buffer (optional; a Sculptor with no power decodes to null), mirroring
 ...
 *   <li>{@code card}: the inserted recipe-script item (optional; null = no script -> the Sculptor is INERT),
 *       omitted on encode when null exactly like the "Energy" object-codec pattern.</li>
 ...
 * {@code MachineBlockState}, so each placed Sculptor receives fully independent containers + buffers.
 */
public final class SculptorState implements Component<ChunkStore>, AutoCraftNode {
    ...
    public static final BuilderCodec<SculptorState> CODEC = BuilderCodec.builder(SculptorState.class, SculptorState::new)
        // Energy is optional: a Sculptor may carry no power. EnergyBuffer.CODEC is an object codec, so a
        ...
        // The inserted recipe-script item: optional. ItemStack.CODEC is an object codec, so a null value is
        ...
        // Sculptor has no cursor, so the key is omitted and the field stays null. Codec.STRING is an object
        ...
        // The recipe currently being crafted. OPTIONAL (3-arg, not-required): an idle Sculptor has no active
    ...
    /** The inserted recipe-script item, or null if no script is loaded (then the Sculptor is INERT). See "Card". */
    ...
    public SculptorState() {
    }

    // --- energy/ports accessors (mirror MachineBlockState) ---

    /** @return the energy handler, or null if this Sculptor carries no power. */
    @Override
    ...
    /** Replaces this Sculptor's port configuration. A null config resets to empty. */
    public void setPorts(PortConfig ports) {
        this.ports = ports != null ? ports : PortConfig.of(List.of());
    }

    // --- container accessors (the Sculptor owns these directly) ---
    ...
    /** @return the inserted recipe-script item, or null if no script is loaded (then the Sculptor is INERT). */
    @Override
    public ItemStack card() {
        return card;
    }

    /** Sets (or clears, when null) the inserted recipe-script item. */
    ...
    /**
     * @return whether this Sculptor is ON (enabled). When OFF, the tick holds it: no crafting, but its
