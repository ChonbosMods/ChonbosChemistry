...
/**
 * Default {@link PortConfig} for the Outfitter machine block (an autonomous crafter with no vanilla bench;
 * its state lives in {@link OutfitterState}, not {@code MachineBlockState}).
 *
 * <p>The Outfitter is a 2(X)×2(Y)×1(Z) footprint (2 wide like the Smelter), anchor at the East-bottom corner
 ...
 * the Outfitter model's Port_ItemIn/Port_ItemOut/Port_Power.
 */
public final class OutfitterPorts {

    private OutfitterPorts() {
    }

    /**
     * @return the default Outfitter port layout (mirrors {@code ForgePorts}/{@code SmelterPorts}): item-out on
