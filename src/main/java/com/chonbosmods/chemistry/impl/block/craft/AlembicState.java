...
/**
 * The persistent ECS state the Alembic block carries on the {@link ChunkStore}. Unlike the processing
 * machines (Smelter/Reclaimer) whose {@code MachineBlockState} HOLDS a vanilla
 * {@code ProcessingBenchBlock} to borrow its input/output containers, the Alembic has no vanilla bench
 * block to delegate to: it is an autonomous crafter and therefore owns its own containers and craft
 * state here.
 *
 * <h2>Fields</h2>
 * <ul>
 *   <li>{@code energy}: the power buffer (optional; a Alembic with no power decodes to null), mirroring
 ...
 * {@code MachineBlockState}, so each placed Alembic receives fully independent containers + buffers.
 */
public final class AlembicState implements Component<ChunkStore>, AutoCraftNode {
    ...
    public static final BuilderCodec<AlembicState> CODEC = BuilderCodec.builder(AlembicState.class, AlembicState::new)
        // Energy is optional: a Alembic may carry no power. EnergyBuffer.CODEC is an object codec, so a
        ...
        // Alembic has no cursor, so the key is omitted and the field stays null. Codec.STRING is an object
        ...
        // The recipe currently being crafted. OPTIONAL (3-arg, not-required): an idle Alembic has no active
    ...
    /** @return the energy handler, or null if this Alembic carries no power. */
    @Override
    ...
    /** Replaces this Alembic's port configuration. A null config resets to empty. */
    public void setPorts(PortConfig ports) {
        this.ports = ports != null ? ports : PortConfig.of(List.of());
    }

    // --- container accessors (the Alembic owns these directly) ---
    ...
    /**
     * @return whether this Alembic is ON (enabled). When OFF, the tick holds it: no crafting, but its
