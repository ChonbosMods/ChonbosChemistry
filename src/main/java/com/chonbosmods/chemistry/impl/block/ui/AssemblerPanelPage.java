...
/**
 * READ-ONLY panel for the CC Assembler ({@code Pages/CC_AssemblerPanel.ui}). It is the sibling of
 * {@link BenchMachinePanelPage} (Smelter/Reclaimer) but reads a {@link AssemblerState} rather than a
 * {@code MachineBlockState}: the Assembler is an AUTONOMOUS crafter, so it owns its input/output containers,
 ...
 *
 * <p>The left "input" grid is NOT a push-fed input buffer: the Assembler is a demand-driven puller, so its
 ...
 * {@link AssemblerState#setEnabled(boolean)}) and an Eject button draining the Assembler's own input + output
 ...
 * {@link AssemblerTickSystem#craftDurationFor} (the same per-recipe {@code getTimeSeconds}, or
 * {@code ASSEMBLER_DEFAULT_DURATION} fallback, the tick times the craft by). So the bar fills exactly over each
 ...
 * on the START tick the ingredients are pulled but progress is still 0, yet the Assembler IS crafting, so
 ...
 */
public final class AssemblerPanelPage extends InteractiveCustomUIPage<AssemblerPanelPage.PageData>
        implements PanelRefreshService.LivePanel {

    @Nonnull
    private final Ref<ChunkStore> blockRef;
    /** Assembler display name shown in the title bar. */
    @Nonnull
    ...
    public void build(
        ...
        commandBuilder.append("Pages/CC_AssemblerPanel.ui");
    ...
    public boolean refresh() {
        ...
        if (this.assembler() == null) {
            this.close(); // assembler broken mid-view: auto-close
            return false;
        }
    ...
    private void applyAction(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull String action) {
        AssemblerState assembler = this.assembler();
        if (assembler == null) {
            return;
        }
        if (Action.TOGGLE.equals(action)) {
            assembler.setEnabled(!assembler.isEnabled());
            this.markNeedsSaving();
        } else if (Action.EJECT.equals(action)) {
            this.eject(ref, store, assembler);
            this.markNeedsSaving();
        } else {
            return;
        }
    ...
    /** Resolve the Assembler's {@link AssemblerState}, or null if the block is gone / not a Assembler. */
    private AssemblerState assembler() {
        if (!this.blockRef.isValid()) {
            return null;
        }
        return this.blockRef.getStore()
            .getComponent(this.blockRef, ChonbosChemistry.getInstance().assemblerComponentType());
    }

    /** Populate every panel selector from the current Assembler state. Null-safe throughout. */
    private void applyState(@Nonnull UICommandBuilder cmd) {
        AssemblerState assembler = this.assembler();
        if (assembler == null) {
            return;
        }
        boolean enabled = assembler.isEnabled();
        ...
        float progressSeconds = assembler.progress();
        boolean active = assembler.currentRecipeId() != null;
        float duration = AssemblerTickSystem.craftDurationFor(assembler.currentRecipeId());
        ...
        long stored = 0;
        long capacity = 0;
        EnergyHandler energy = assembler.energy();
        if (energy != null) {
     ...
     * the held ingredients list so an idle Assembler renders an empty list and a crafting Assembler renders just the
    ...
    /** Drain the Assembler's own input + output containers into the player's inventory; drop overflow at feet. */
    private void eject(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull AssemblerState assembler) {
        ...
        List<ContainerView> sources = List.of(
            new ItemContainerView(assembler.held()),
            new ItemContainerView(assembler.output()));
        ...
        // keeps both the ingredients and the finished item (item duplication). Resetting here drops the Assembler
        ...
        assembler.setCurrentRecipeId(null);
        assembler.setProgress(0f);
        ...
        if (card != null && !ItemStack.isEmpty(card)) {
            ...
            assembler.setCard(null);
            assembler.clearScriptProgress();
        }
        ...
        if (transform == null || transform.getPosition() == null) {
            return; // can't place a drop without a position; the items stay in the Assembler
        }
    ...
    /** Mark the Assembler's chunk needing-save after a toggle/eject (WrenchInteraction pattern). */
