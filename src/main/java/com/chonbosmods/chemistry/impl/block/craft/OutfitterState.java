 ...
 * {@code ProcessingBenchBlock} to borrow its input/output containers, the Outfitter has no vanilla bench
 * block to delegate to: it is an autonomous crafter and therefore owns its own containers and craft
 * state here.
 *
 * <h2>Fields</h2>
 * <ul>
 *   <li>{@code energy}: the power buffer (optional; a Outfitter with no power decodes to null), mirroring
 ...
 * {@code MachineBlockState}, so each placed Outfitter receives fully independent containers + buffers.
 */
public final class OutfitterState implements Component<ChunkStore>, AutoCraftNode {
    ...
    public static final BuilderCodec<OutfitterState> CODEC = BuilderCodec.builder(OutfitterState.class, OutfitterState::new)
        // Energy is optional: a Outfitter may carry no power. EnergyBuffer.CODEC is an object codec, so a
        ...
        // Outfitter has no cursor, so the key is omitted and the field stays null. Codec.STRING is an object
        ...
        // The recipe currently being crafted. OPTIONAL (3-arg, not-required): an idle Outfitter has no active
    ...
    /** @return the energy handler, or null if this Outfitter carries no power. */
    @Override
    ...
    /** Replaces this Outfitter's port configuration. A null config resets to empty. */
    public void setPorts(PortConfig ports) {
        this.ports = ports != null ? ports : PortConfig.of(List.of());
    }

    // --- container accessors (the Outfitter owns these directly) ---
    ...
    /**
     * @return whether this Outfitter is ON (enabled). When OFF, the tick holds it: no crafting, but its
