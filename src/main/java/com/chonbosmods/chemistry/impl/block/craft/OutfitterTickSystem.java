...
/**
 * Per-tick driver for the autonomous Outfitter block (a PURE auto-craft machine). For every live
 * {@link OutfitterState} on the {@link ChunkStore}, it resolves the live drive context (ref / world / block
 ...
 * {@link #OUTFITTER_SPEC} (the Outfitter's bench pool, energy draw, post-craft delay, and the two improvement
 * hooks). Unlike the Forge, the Outfitter's pool unions BOTH a Tannery processing bench AND a Loom
 * crafting bench: both are {@link CraftingRecipe} objects, so the engine reads input&rarr;output off either,
 * and each recipe crafts for its own time via {@link VanillaCraftBridge#recipeTimeSeconds}.
 *
 ...
 */
public final class OutfitterTickSystem extends EntityTickingSystem<ChunkStore> {

    /** [TUNE] Per-second energy draw of a running Outfitter. Placeholder, matches {@code FORGE_DRAW}. */
    private static final long OUTFITTER_DRAW = 200L;

    /**
     * [TUNE] Fallback seconds one craft takes for instant (time-0) crafting recipes. Tannery tans use
     * their real {@link VanillaCraftBridge#recipeTimeSeconds}; Loom dishes are instant in vanilla so they
     * fall back to this.
     *
     ...
     */
    public static final float OUTFITTER_DEFAULT_DURATION = 4.0f;
     ...
     */
    private static final float OUTFITTER_TIME_MULTIPLIER = 2.0f;

    /**
     * [TUNE] Ticks the Outfitter idles after completing a craft before sourcing the next recipe. A cosmetic
    ...
    public void tick(
        ...
        OutfitterState node = archetypeChunk.getComponent(index, outfitterType);
        if (node == null) {
            return;
        }
        // DEFENSIVE: a craft drive error must NEVER kill the WorldThread. Catch every throwable, log each
        // distinct one once, and skip this Outfitter's drive for the tick (same contract as MachineTickSystem).
        try {
            driveCraft(node, dt, index, archetypeChunk, store);
        } catch (Throwable t) {
            String msg = String.valueOf(t);
            if (OUTFITTER_DRIVE_SEEN.add(msg)) {
                OUTFITTER_DRIVE_LOG.atWarning().log("CC outfitter craft drive failed (skipped, world tick protected): " + msg);
            }
        }
    }

    /** Guards the per-tick craft drive: a failure logs once (per distinct error) and never crashes the tick. */
    private static final HytaleLogger OUTFITTER_DRIVE_LOG = HytaleLogger.forEnclosingClass();
    private static final java.util.Set<String> OUTFITTER_DRIVE_SEEN =
        java.util.concurrent.ConcurrentHashMap.newKeySet();

    /**
     * Resolve this Outfitter node's live drive context (the prologue) and hand it to {@link AutoCraftEngine}. A
    ...
    private void driveCraft(
        ...
        // the block visual swap, configured by OUTFITTER_SPEC. The engine never throws (the tick's catch is the
        // backstop).
        ...
        AutoCraftEngine.drive(node, engineCtx, OUTFITTER_SPEC);
    }

    // --- the Outfitter's engine configuration (the machine-specific Spec) ---

    /**
     * The Outfitter's {@link AutoCraftEngine.Spec}: its bench pool (cached on first use), energy draw,
     * post-craft delay, and the two improvement hooks. {@code craftDuration} returns the recipe's real craft
     * time ({@link VanillaCraftBridge#recipeTimeSeconds}) or {@link #OUTFITTER_DEFAULT_DURATION} for instant
     * (time-0) crafting recipes; {@code allowSet} delegates to {@link AutoCraftEngine#cardAllowSet}.
     */
    private static final AutoCraftEngine.Spec OUTFITTER_SPEC = new AutoCraftEngine.Spec() {
        @Override
        public RecipePool recipePool() {
            return outfitterRecipePool();
        }

        @Override
        public long energyDraw() {
            return OUTFITTER_DRAW;
        }
        ...
        public float craftDuration(@Nullable CraftingRecipe r) {
            ...
            // every idle tick NPEs. Scaled by OUTFITTER_TIME_MULTIPLIER (the GUI bar reads the same scaled value
            // via craftDurationFor, so bar and tick stay in lockstep).
            float base = (r == null) ? OUTFITTER_DEFAULT_DURATION : VanillaCraftBridge.recipeTimeSeconds(r);
            if (base <= 0f) {
                base = OUTFITTER_DEFAULT_DURATION;
            }
            return base * OUTFITTER_TIME_MULTIPLIER;
        }
    ...
    /**
     * The real craft duration (seconds) of the recipe currently loaded in an Outfitter, for the PANEL's
     ...
     * or {@link #OUTFITTER_DEFAULT_DURATION} for a time-0 recipe), so the bar fills over exactly the craft time the
    ...
    public static float craftDurationFor(@Nullable String recipeId) {
        CraftingRecipe r = recipeId == null ? null : outfitterRecipePool().map().get(recipeId);
        return OUTFITTER_SPEC.craftDuration(r); // null-guarded -> OUTFITTER_DEFAULT_DURATION
    }
    ...
    /**
     * The Outfitter's shared recipe pool, built once on first tick and cached (the recipe registry is
     * load-time-fixed). Unions the Tannery processing bench + the Loom crafting bench and dedups by id
     * into a deterministic (natural String) order via {@link RecipePool#union}.
     */
    private static RecipePool outfitterRecipePool() {
        RecipePool p = POOL;
        if (p != null) {
            return p;
        }
        synchronized (OutfitterTickSystem.class) {
            if (POOL != null) {
                return POOL;
            }
            RecipePool pool = RecipePool.union(List.of(
                new RecipePool.BenchRef(BenchType.Processing, "Tannery"),
                new RecipePool.BenchRef(BenchType.Crafting, "Loombench")));
            POOL = pool;
            // Diagnostic (logged once): if this is 0, the bench ids did not resolve to recipes and the Outfitter
            // can never craft. Confirms the Tannery + Loom benches resolved.
            OUTFITTER_DRIVE_LOG.atInfo().log("Outfitter recipe pool built: " + pool.stableOrder().size() + " recipes.");
            return POOL;
        }
    }
}
