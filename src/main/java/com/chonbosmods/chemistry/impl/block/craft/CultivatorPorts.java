...
/**
 * Default {@link PortConfig} for the Cultivator machine block (an autonomous crafter with no vanilla bench;
 * its state lives in {@link CultivatorState}, not {@code MachineBlockState}).
 *
 * <p>The Cultivator is a 2(X)×2(Y)×1(Z) footprint (2 wide like the Smelter), anchor at the East-bottom corner
 ...
 * the Cultivator model's Port_ItemIn/Port_ItemOut/Port_Power.
 */
public final class CultivatorPorts {

    private CultivatorPorts() {
    }

    /**
     * @return the default Cultivator port layout (mirrors {@code ForgePorts}/{@code SmelterPorts}): item-out on
