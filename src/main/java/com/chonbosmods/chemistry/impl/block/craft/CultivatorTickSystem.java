...
/**
 * Per-tick driver for the autonomous Cultivator block (a PURE auto-craft machine). For every live
 * {@link CultivatorState} on the {@link ChunkStore}, it resolves the live drive context (ref / world / block
 ...
 * {@link #CULTIVATOR_SPEC} (the Cultivator's bench pool, energy draw, post-craft delay, and the two improvement
 * hooks). The Cultivator's pool sources the single Farmingbench ({@link BenchType#Crafting}): it yields
 * {@link CraftingRecipe} objects, so the engine reads input&rarr;output off them, and each recipe crafts for
 * its own time via {@link VanillaCraftBridge#recipeTimeSeconds}.
 *
 ...
 */
public final class CultivatorTickSystem extends EntityTickingSystem<ChunkStore> {

    /** [TUNE] Per-second energy draw of a running Cultivator. Placeholder, matches {@code FORGE_DRAW}. */
    private static final long CULTIVATOR_DRAW = 200L;
     ...
     */
    public static final float CULTIVATOR_DEFAULT_DURATION = 4.0f;
     ...
     */
    private static final float CULTIVATOR_TIME_MULTIPLIER = 2.0f;

    /**
     * [TUNE] Ticks the Cultivator idles after completing a craft before sourcing the next recipe. A cosmetic
    ...
    public void tick(
        ...
        CultivatorState node = archetypeChunk.getComponent(index, cultivatorType);
        if (node == null) {
            return;
        }
        // DEFENSIVE: a craft drive error must NEVER kill the WorldThread. Catch every throwable, log each
        // distinct one once, and skip this Cultivator's drive for the tick (same contract as MachineTickSystem).
        try {
            driveCraft(node, dt, index, archetypeChunk, store);
        } catch (Throwable t) {
            String msg = String.valueOf(t);
            if (CULTIVATOR_DRIVE_SEEN.add(msg)) {
                CULTIVATOR_DRIVE_LOG.atWarning().log("CC cultivator craft drive failed (skipped, world tick protected): " + msg);
            }
        }
    }

    /** Guards the per-tick craft drive: a failure logs once (per distinct error) and never crashes the tick. */
    private static final HytaleLogger CULTIVATOR_DRIVE_LOG = HytaleLogger.forEnclosingClass();
    private static final java.util.Set<String> CULTIVATOR_DRIVE_SEEN =
        java.util.concurrent.ConcurrentHashMap.newKeySet();

    /**
     * Resolve this Cultivator node's live drive context (the prologue) and hand it to {@link AutoCraftEngine}. A
    ...
    private void driveCraft(
        ...
        // the block visual swap, configured by CULTIVATOR_SPEC. The engine never throws (the tick's catch is the
        // backstop).
        ...
        AutoCraftEngine.drive(node, engineCtx, CULTIVATOR_SPEC);
    }

    // --- the Cultivator's engine configuration (the machine-specific Spec) ---

    /**
     * The Cultivator's {@link AutoCraftEngine.Spec}: its bench pool (cached on first use), energy draw,
     * post-craft delay, and the two improvement hooks. {@code craftDuration} returns the recipe's real craft
     * time ({@link VanillaCraftBridge#recipeTimeSeconds}) or {@link #CULTIVATOR_DEFAULT_DURATION} for instant
     * (time-0) crafting recipes; {@code allowSet} delegates to {@link AutoCraftEngine#cardAllowSet}.
     */
    private static final AutoCraftEngine.Spec CULTIVATOR_SPEC = new AutoCraftEngine.Spec() {
        @Override
        public RecipePool recipePool() {
            return cultivatorRecipePool();
        }

        @Override
        public long energyDraw() {
            return CULTIVATOR_DRAW;
        }
        ...
        public float craftDuration(@Nullable CraftingRecipe r) {
            ...
            // every idle tick NPEs. Scaled by CULTIVATOR_TIME_MULTIPLIER (the GUI bar reads the same scaled value
            // via craftDurationFor, so bar and tick stay in lockstep).
            float base = (r == null) ? CULTIVATOR_DEFAULT_DURATION : VanillaCraftBridge.recipeTimeSeconds(r);
            if (base <= 0f) {
                base = CULTIVATOR_DEFAULT_DURATION;
            }
            return base * CULTIVATOR_TIME_MULTIPLIER;
        }
    ...
    /**
     * The real craft duration (seconds) of the recipe currently loaded in an Cultivator, for the PANEL's
     ...
     * or {@link #CULTIVATOR_DEFAULT_DURATION} for a time-0 recipe), so the bar fills over exactly the craft time the
    ...
    public static float craftDurationFor(@Nullable String recipeId) {
        CraftingRecipe r = recipeId == null ? null : cultivatorRecipePool().map().get(recipeId);
        return CULTIVATOR_SPEC.craftDuration(r); // null-guarded -> CULTIVATOR_DEFAULT_DURATION
    }
    ...
    /**
     * The Cultivator's shared recipe pool, built once on first tick and cached (the recipe registry is
     * load-time-fixed). Sources the single Farmingbench ({@link BenchType#Crafting}) and dedups by id into a
     * deterministic (natural String) order via {@link RecipePool#union}. The vanilla Trough bench shares the
     * same {@code Farmingbench} id, so this single ref already covers both farming + trough goods (no separate
     * Trough ref needed).
     */
    private static RecipePool cultivatorRecipePool() {
        RecipePool p = POOL;
        if (p != null) {
            return p;
        }
        synchronized (CultivatorTickSystem.class) {
            if (POOL != null) {
                return POOL;
            }
            RecipePool pool = RecipePool.union(List.of(
                new RecipePool.BenchRef(BenchType.Crafting, "Farmingbench")));
            POOL = pool;
            // Diagnostic (logged once): if this is 0, the bench id did not resolve to recipes and the Cultivator
            // can never craft. Confirms the Farmingbench resolved.
            CULTIVATOR_DRIVE_LOG.atInfo().log("Cultivator recipe pool built: " + pool.stableOrder().size() + " recipes.");
            return POOL;
        }
    }
}
