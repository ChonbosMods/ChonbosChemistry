...
/**
 * The persistent ECS state the Cultivator block carries on the {@link ChunkStore}. Unlike the processing
 * machines (Smelter/Reclaimer) whose {@code MachineBlockState} HOLDS a vanilla
 * {@code ProcessingBenchBlock} to borrow its input/output containers, the Cultivator has no vanilla bench
 * block to delegate to: it is an autonomous crafter and therefore owns its own containers and craft
 * state here.
 *
 * <h2>Fields</h2>
 * <ul>
 *   <li>{@code energy}: the power buffer (optional; a Cultivator with no power decodes to null), mirroring
 ...
 * {@code MachineBlockState}, so each placed Cultivator receives fully independent containers + buffers.
 */
public final class CultivatorState implements Component<ChunkStore>, AutoCraftNode {
    ...
    public static final BuilderCodec<CultivatorState> CODEC = BuilderCodec.builder(CultivatorState.class, CultivatorState::new)
        // Energy is optional: a Cultivator may carry no power. EnergyBuffer.CODEC is an object codec, so a
        ...
        // Cultivator has no cursor, so the key is omitted and the field stays null. Codec.STRING is an object
        ...
        // The recipe currently being crafted. OPTIONAL (3-arg, not-required): an idle Cultivator has no active
    ...
    public CultivatorState() {
    }

    // --- energy/ports accessors (mirror MachineBlockState) ---

    /** @return the energy handler, or null if this Cultivator carries no power. */
    @Override
    ...
    /** Replaces this Cultivator's port configuration. A null config resets to empty. */
    public void setPorts(PortConfig ports) {
        this.ports = ports != null ? ports : PortConfig.of(List.of());
    }

    // --- container accessors (the Cultivator owns these directly) ---
    ...
    /**
     * @return whether this Cultivator is ON (enabled). When OFF, the tick holds it: no crafting, but its
