...
/**
 * Default {@link PortConfig} for the Assembler machine block (an autonomous crafter with no vanilla bench;
 * its state lives in {@link AssemblerState}, not {@code MachineBlockState}).
 *
 * <p>The Assembler is a 2(X)×2(Y)×1(Z) footprint (2 wide like the Smelter), anchor at the East-bottom corner
 ...
 * the Assembler model's Port_ItemIn/Port_ItemOut/Port_Power.
 */
public final class AssemblerPorts {

    private AssemblerPorts() {
    }

    /**
     * @return the default Assembler port layout (mirrors {@code ForgePorts}/{@code SmelterPorts}): item-out on
