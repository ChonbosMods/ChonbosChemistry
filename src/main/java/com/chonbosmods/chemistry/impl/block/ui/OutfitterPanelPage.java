...
/**
 * READ-ONLY panel for the CC Outfitter ({@code Pages/CC_OutfitterPanel.ui}). It is the sibling of
 * {@link BenchMachinePanelPage} (Smelter/Reclaimer) but reads a {@link OutfitterState} rather than a
 * {@code MachineBlockState}: the Outfitter is an AUTONOMOUS crafter, so it owns its input/output containers,
 ...
 *
 * <p>The left "input" grid is NOT a push-fed input buffer: the Outfitter is a demand-driven puller, so its
 ...
 * {@link OutfitterState#setEnabled(boolean)}) and an Eject button draining the Outfitter's own input + output
 ...
 * {@link OutfitterTickSystem#craftDurationFor} (the same per-recipe {@code getTimeSeconds}, or
 * {@code OUTFITTER_DEFAULT_DURATION} fallback, the tick times the craft by). So the bar fills exactly over each
 ...
 * on the START tick the ingredients are pulled but progress is still 0, yet the Outfitter IS crafting, so
 ...
 */
public final class OutfitterPanelPage extends InteractiveCustomUIPage<OutfitterPanelPage.PageData>
        implements PanelRefreshService.LivePanel {

    @Nonnull
    private final Ref<ChunkStore> blockRef;
    /** Outfitter display name shown in the title bar. */
    @Nonnull
    ...
    public void build(
        ...
        commandBuilder.append("Pages/CC_OutfitterPanel.ui");
    ...
    public boolean refresh() {
        ...
        if (this.outfitter() == null) {
            this.close(); // outfitter broken mid-view: auto-close
            return false;
        }
    ...
    private void applyAction(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull String action) {
        OutfitterState outfitter = this.outfitter();
        if (outfitter == null) {
            return;
        }
        if (Action.TOGGLE.equals(action)) {
            outfitter.setEnabled(!outfitter.isEnabled());
            this.markNeedsSaving();
        } else if (Action.EJECT.equals(action)) {
            this.eject(ref, store, outfitter);
            this.markNeedsSaving();
        } else {
            return;
        }
    ...
    /** Resolve the Outfitter's {@link OutfitterState}, or null if the block is gone / not a Outfitter. */
    private OutfitterState outfitter() {
        if (!this.blockRef.isValid()) {
            return null;
        }
        return this.blockRef.getStore()
            .getComponent(this.blockRef, ChonbosChemistry.getInstance().outfitterComponentType());
    }

    /** Populate every panel selector from the current Outfitter state. Null-safe throughout. */
    private void applyState(@Nonnull UICommandBuilder cmd) {
        OutfitterState outfitter = this.outfitter();
        if (outfitter == null) {
            return;
        }
        boolean enabled = outfitter.isEnabled();
        ...
        float progressSeconds = outfitter.progress();
        boolean active = outfitter.currentRecipeId() != null;
        float duration = OutfitterTickSystem.craftDurationFor(outfitter.currentRecipeId());
        ...
        long stored = 0;
        long capacity = 0;
        EnergyHandler energy = outfitter.energy();
        if (energy != null) {
     ...
     * the held ingredients list so an idle Outfitter renders an empty list and a crafting Outfitter renders just the
    ...
    /** Drain the Outfitter's own input + output containers into the player's inventory; drop overflow at feet. */
    private void eject(@Nonnull Ref<EntityStore> ref, @Nonnull Store<EntityStore> store, @Nonnull OutfitterState outfitter) {
        ...
        List<ContainerView> sources = List.of(
            new ItemContainerView(outfitter.held()),
            new ItemContainerView(outfitter.output()));
        ...
        // keeps both the ingredients and the finished item (item duplication). Resetting here drops the Outfitter
        ...
        if (transform == null || transform.getPosition() == null) {
            return; // can't place a drop without a position; the items stay in the Outfitter
        }
    ...
    /** Mark the Outfitter's chunk needing-save after a toggle/eject (WrenchInteraction pattern). */
