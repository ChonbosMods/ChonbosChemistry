...
/**
 * Default {@link PortConfig} for the Cooker machine block (an autonomous crafter with no vanilla bench;
 * its state lives in {@link CookerState}, not {@code MachineBlockState}).
 *
 * <p>The Cooker is a 2(X)×2(Y)×1(Z) footprint (2 wide like the Smelter), anchor at the East-bottom corner
 ...
 * the Cooker model's Port_ItemIn/Port_ItemOut/Port_Power.
 */
public final class CookerPorts {

    private CookerPorts() {
    }

    /**
     * @return the default Cooker port layout (mirrors {@code ForgePorts}/{@code SmelterPorts}): item-out on
