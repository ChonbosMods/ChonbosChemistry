...
/**
 * The persistent ECS state the Forge block carries on the {@link ChunkStore}. Unlike the processing
 * machines (Smelter/Reclaimer) whose {@code MachineBlockState} HOLDS a vanilla
 * {@code ProcessingBenchBlock} to borrow its input/output containers, the Forge has no vanilla bench
...
public final class ForgeCraftState implements Component<ChunkStore> {
    ...
    public static final BuilderCodec<ForgeCraftState> CODEC = BuilderCodec.builder(ForgeCraftState.class, ForgeCraftState::new)
        // Energy is optional: a Forge may carry no power. EnergyBuffer.CODEC is an object codec, so a
        ...
        // Forge has no cursor, so the key is omitted and the field stays null. Codec.STRING is an object
    ...
    /** Replaces this Forge's port configuration. A null config resets to empty. */
