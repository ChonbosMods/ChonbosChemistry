...
class NetworkEndpointsTest {
    ...
    /** A single POWER pipe at (x,y,z), all faces NORMAL, returned on every lookup. */
    private static PipeGridView powerPipeAllNormal(int x, int y, int z) {
        long key = NetworkManager.packKey(x, y, z);
        PipeNode node = PipeNode.of(PortChannel.POWER, 1);
        return (px, py, pz) -> NetworkManager.packKey(px, py, pz) == key ? node : null;
    }

    /** An OUTPUT-power machine whose facing port sits on {@code facingFace}. */
    private static FakePorts outputMachine(int facingFace) {
        return new FakePorts(
            portConfig(facingFace, PortChannel.POWER, PortDirection.OUTPUT),
            EnergyBuffer.withCapacity(1000L), Map.of());
    }

    @Test
    void cap_fourMachinesAroundOnePowerPipe_onlyThreeLowestFacesCollected() {
        // Pipe (5,5,5) with OUTPUT machines on faces 0(+X),1(-X),2(+Y),3(-Y). Cap = 3: faces 0,1,2 win,
        // face 3 (the 4th) is dropped entirely.
        PipeGridView grid = powerPipeAllNormal(5, 5, 5);
        Network net = new NetworkManager().getOrBuildNetwork(5, 5, 5, grid);
        FakeLookup lookup = new FakeLookup();
        lookup.put(6, 5, 5, outputMachine(1)); // face 0 +X, facing -X(1)
        lookup.put(4, 5, 5, outputMachine(0)); // face 1 -X, facing +X(0)
        lookup.put(5, 6, 5, outputMachine(3)); // face 2 +Y, facing -Y(3)
        lookup.put(5, 4, 5, outputMachine(2)); // face 3 -Y, facing +Y(2) -- the 4th, dropped

        NetworkEndpoints.Endpoints endpoints = NetworkEndpoints.collect(net, grid, lookup);
        assertEquals(3, endpoints.pureProviders().size(), "cap drops the 4th machine");
    }

    @Test
    void cap_noneFaceDoesNotConsumeBudget() {
        // Same four machines but face 1 (-X) pipe face is NONE: NONE hides its machine and burns no
        // budget, so the qualifying faces 0,2,3 (three) all collect.
        long key = NetworkManager.packKey(5, 5, 5);
        PipeNode node = PipeNode.of(PortChannel.POWER, 1);
        node.setFlowState(1, FlowState.NONE);
        PipeGridView grid = (px, py, pz) -> NetworkManager.packKey(px, py, pz) == key ? node : null;
        Network net = new NetworkManager().getOrBuildNetwork(5, 5, 5, grid);
        FakeLookup lookup = new FakeLookup();
        lookup.put(6, 5, 5, outputMachine(1)); // face 0 collects
        lookup.put(4, 5, 5, outputMachine(0)); // face 1 NONE: hidden, no budget
        lookup.put(5, 6, 5, outputMachine(3)); // face 2 collects
        lookup.put(5, 4, 5, outputMachine(2)); // face 3 collects (would drop if NONE counted)

        NetworkEndpoints.Endpoints endpoints = NetworkEndpoints.collect(net, grid, lookup);
        assertEquals(3, endpoints.pureProviders().size(),
            "NONE face consumes no budget; the three reachable machines all collect");
    }

    @Test
    void cap_storageCountsAsOneConnection() {
        // Pipe (5,5,5): a storage (BOTH across NORMAL) on face 0 plus OUTPUT machines on faces 1,2,3.
        // The storage is ONE connected neighbour (it contributes provider+acceptor halves) so it claims
        // budget ONCE: faces 0,1,2 win, face 3 is the 4th and drops.
        PipeGridView grid = powerPipeAllNormal(5, 5, 5);
        Network net = new NetworkManager().getOrBuildNetwork(5, 5, 5, grid);
        FakeLookup lookup = new FakeLookup();
        lookup.put(6, 5, 5, new FakePorts(
            portConfig(1, PortChannel.POWER, PortDirection.BOTH),
            EnergyBuffer.withCapacity(1000L), Map.of())); // face 0 storage = 1 connection
        lookup.put(4, 5, 5, outputMachine(0)); // face 1
        lookup.put(5, 6, 5, outputMachine(3)); // face 2
        lookup.put(5, 4, 5, outputMachine(2)); // face 3 -- 4th, dropped

        NetworkEndpoints.Endpoints endpoints = NetworkEndpoints.collect(net, grid, lookup);
        assertEquals(1, endpoints.storages().size(), "storage on face 0 collected (counts as one)");
        assertEquals(2, endpoints.pureProviders().size(), "faces 1,2 collect; face 3 dropped by the cap");
    }

    @Test
    void cap_isPerPipeNotPerNetwork_twoPipesThreeEach() {
        // Two connected POWER pipes (5,5,5)-(6,5,5). Each borders 3 OTHER OUTPUT machines. Per-pipe
        // budget => 3 each = 6 providers, none dropped.
        ...
        Network net = new NetworkManager().getOrBuildNetwork(5, 5, 5, grid);
        FakeLookup lookup = new FakeLookup();
        // pipe a (5,5,5): machines on -X(face1), +Y(face2), -Y(face3)
        lookup.put(4, 5, 5, outputMachine(0));
        lookup.put(5, 6, 5, outputMachine(3));
        lookup.put(5, 4, 5, outputMachine(2));
        // pipe b (6,5,5): machines on +X(face0), +Y(face2), -Y(face3)
        lookup.put(7, 5, 5, outputMachine(1));
        lookup.put(6, 6, 5, outputMachine(3));
        lookup.put(6, 4, 5, outputMachine(2));

        NetworkEndpoints.Endpoints endpoints = NetworkEndpoints.collect(net, grid, lookup);
        assertEquals(6, endpoints.pureProviders().size(),
            "per-pipe cap: two pipes x 3 each = 6");
    }
